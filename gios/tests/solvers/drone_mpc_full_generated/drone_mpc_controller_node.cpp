...
AcadoSolver::AcadoSolver(const AcadoSolver::Parameters parameters){//{{{ Constructor
  ...
  //for( unsigned i=0; i<ACADO_NY; i++){
	// 	acadoVariables.W[ ( i )*(ACADO_NY+1) ] = 1;
  //}
...
void AcadoSolver::setObstacle(const EllipsoidalObstacle &obstacle_){//{{{ Static obstacle
  for (unsigned i = 0 ; i < n + 1 ; i++){
    ...
    acadoVariables.od[ od_eobs_offset + 15 + od_eobs_p*obstacle_.id + i*nod ] = obstacle_.s_k;//Sensitivity
  	...
  	acadoVariables.W[ (y_eobs_offset + obstacle_.id)*(ny+1) ] = obstacle_.s_w;//Weight
 // for (unsigned i = 0 ; i < n ; i++){
...
void AcadoSolver::setObstacle(const PlanarObstacle &obstacle_){//{{{
  ...
  acadoVariables.W[ (y_pobs_offset + obstacle_.id)*(ny+1) ] = obstacle_.s_w;//Weight
//  for (unsigned i = 0 ; i < n ; i++){
...
void AcadoSolver::setBoundsMax(const State &max_state_){//{{{Static bounds
  ...
  acadoVariables.W[ (y_minmax_offset + 0 )*(ny+1) ] = 1;//Weight
  acadoVariables.W[ (y_minmax_offset + 1 )*(ny+1) ] = 1;//Weight
  acadoVariables.W[ (y_minmax_offset + 2 )*(ny+1) ] = 1;//Weight
  acadoVariables.W[ (y_minmax_offset + 3 )*(ny+1) ] = 1;//Weight
};//}}}
...
int main(){
  ...
  for( unsigned i=0; i<ACADO_NY; i++){
	 	printf("W %i: %f\n",i,acadoVariables.W[ ( i )*(ACADO_NY+1) ]);
  }
  return 0;
 ...
 //	acado_printControlVariables();
  ...
  initial_reference.x = 1;
  initial_reference.y = 1;
  initial_reference.z = 1;
...
//  }
//  for( unsigned i=0; i< nyn*nyn; i++) acadoVariables.WN[i] = 0;
//    
//  for( unsigned i=0; i< nyn; i++){
//    acadoVariables.WN[i*(nyn +1)] = 1;
//  }
